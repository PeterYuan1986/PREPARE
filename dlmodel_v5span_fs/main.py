...
def preprocess_onehot(train_features, train_labels,  predictyear):
    ...
    testing_df1 = testing_df.dropna(subset=['age_03'])
    str_feature = []
    dic['drop_feature'] = []
    for x in testing_df.columns:
        ratio = testing_df[x].isna().sum() / len(testing_df[x])
        ratio_ = testing_df1[x].isna().sum() / len(testing_df1[x])
        if ratio > 0.8 or ratio_ > 0.8:
    ...
    df.to_csv(
        f"./processed_onehot_{str(predictyear)}.csv")
    data = df.drop(
        ['year', 'uid'], axis=1)
    return data
...
def main():
    ...
    checkpoint_path = f'/STORAGE/peter/PREPARE/dlmodel_v5span_fs/checkpoints_{args.year}_{args.fold}'
    startepoch = 1
    ...
    # Load your dataset
    if not os.path.isfile(f'/STORAGE/peter/PREPARE/dlmodel_v5span_fs/train_{args.year}_{args.fold}.csv') or not os.path.isfile(f'/STORAGE/peter/PREPARE/dlmodel_v5span_fs/valid_{args.year}_{args.fold}.csv'):
        ...
        # Save to CSV files
        train_df.to_csv(
            f'/STORAGE/peter/PREPARE/dlmodel_v5span_fs/train_{args.year}_{args.fold}.csv', index=False)
        valid_df.to_csv(
            f'/STORAGE/peter/PREPARE/dlmodel_v5span_fs/valid_{args.year}_{args.fold}.csv', index=False)

    in_model = len(pd.read_csv(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span_fs/train_{args.year}_{args.fold}.csv").columns)-1
    print(in_model)
    train_loader = create_traindataloader(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span_fs/train_{args.year}_{args.fold}.csv", args.year, batch_size=args.batch_size)
    valid_loader = create_traindataloader(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span_fs/valid_{args.year}_{args.fold}.csv", args.year, batch_size=args.test_batch_size)
    ...
    if args.continue_train:
        if args.epoch > 0:
            ...
            log_message(f"Loaded checkpoint from epoch {startepoch}.", args)
        else:
    ...
    for epoch in range(startepoch, args.epochs + 1):
        ...
        if epoch % 20 == 0:
            ...
            if args.save_model and epoch % 20 == 0:
                ...
                }, f"/STORAGE/peter/PREPARE/dlmodel_v5span_fs/checkpoints_{args.year}_{args.fold}/ckpt_{epoch}.pt")
        # scheduler.step()

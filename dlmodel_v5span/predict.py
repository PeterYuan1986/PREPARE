...
processed_df['mean_0'] = processed_df['cs_2021_0'] + \
    processed_df['cs_2016_0']/processed_df['cs_2021_0'] + \
    processed_df['cs_2016_0']
...
columns = ["cs_2016_0", "cs_2016_1", "cs_2016_2", "cs_2016_3", "cs_2016_4"]
processed_df["composite_score_16"] = (
    (processed_df[columns].sum(axis=1)) / 5).values
columns = ["cs_2021_0", "cs_2021_1", "cs_2021_2", "cs_2021_3", "cs_2021_4"]
processed_df["composite_score_21"] = (
    (processed_df[columns].sum(axis=1)) / 5).values
processed_df['mean'] = ((processed_df['composite_score_21'] +
                         processed_df['composite_score_16'])/2).values


def update_cs_16(row):
    if row['composite_score_16'] > row['mean']:
        return row['composite_score_16'] - 2
    else:
        return row['composite_score_16'] + 2


def update_cs_21(row):
    if row['composite_score_21'] > row['mean']:
        return row['composite_score_21'] - 2
    else:
        return row['composite_score_21'] + 2


processed_df["composite_score_16"] = processed_df.apply(update_cs_16, axis=1)
processed_df["composite_score_21"] = processed_df.apply(update_cs_21, axis=1)
processed_df.to_csv('result.csv')
r_2016 = pd.DataFrame()
r_2016["uid"] = processed_df.index
r_2016["year"] = 2016
r_2016["composite_score"] = processed_df["composite_score_16"].values
r_2021 = pd.DataFrame()
r_2021["uid"] = processed_df.index
r_2021["year"] = 2021
r_2021["composite_score"] = processed_df["composite_score_21"].values
submit = pd.read_csv(

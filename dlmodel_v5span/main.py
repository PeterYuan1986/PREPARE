...
def main():
    ...
    checkpoint_path = f'/STORAGE/peter/PREPARE/dlmodel_v5span/checkpoints_{args.year}_{args.fold}'
    startepoch = 1
    ...
    # Load your dataset
    if not os.path.isfile(f'/STORAGE/peter/PREPARE/dlmodel_v5span/train_{args.year}_{args.fold}.csv') or not os.path.isfile(f'/STORAGE/peter/PREPARE/dlmodel_v5span/valid_{args.year}_{args.fold}.csv'):
        ...
        # Save to CSV files
        train_df.to_csv(
            f'/STORAGE/peter/PREPARE/dlmodel_v5span/train_{args.year}_{args.fold}.csv', index=False)
        valid_df.to_csv(
            f'/STORAGE/peter/PREPARE/dlmodel_v5span/valid_{args.year}_{args.fold}.csv', index=False)

    in_model = len(pd.read_csv(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span/train_{args.year}_{args.fold}.csv").columns)-1
    print(in_model)
    train_loader = create_traindataloader(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span/train_{args.year}_{args.fold}.csv", args.year, batch_size=args.batch_size)
    valid_loader = create_traindataloader(
        f"/STORAGE/peter/PREPARE/dlmodel_v5span/valid_{args.year}_{args.fold}.csv", args.year, batch_size=args.test_batch_size)
    ...
    for epoch in range(startepoch, args.epochs + 1):
        ...
        if epoch % 20 == 0:
            ...
            if args.save_model and epoch % 20 == 0:
                ...
                }, f"/STORAGE/peter/PREPARE/dlmodel_v5span/checkpoints_{args.year}_{args.fold}/ckpt_{epoch}.pt")
        # scheduler.step()
